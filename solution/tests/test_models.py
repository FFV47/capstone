...
class JSONFieldsTestCase(TestCase):
    ...
    def test_json_fields(self):
        ...
        with self.assertRaisesMessage(ValidationError, "test is not a valid job type"):
            Job.objects.create(types=["test"], shifts=["Day Shift"], tags=["Temporary"], **fields)

        with self.assertRaisesMessage(ValidationError, "test is not a valid shift"):
            Job.objects.create(types=["Overtime"], shifts=["test"], tags=["Temporary"], **fields)

        with self.assertRaisesMessage(ValidationError, "test is not a valid tag"):
            Job.objects.create(
                types=["Overtime"], shifts=["12 Hour Shift"], tags=["test"], **fields
            )
...
# class JobReviewTestCase(TestCase):
#     @classmethod
#     def setUpTestData(cls) -> None:
